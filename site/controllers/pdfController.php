...
function banlance_cta_pagar_detallado($rif){
	...
	$_pdf->line(10,45+(count($this->_proveedor->_factura)*4),($_pdf->w)-7,45+(count($this->_proveedor->_factura)*4));
		$_pdf->Cell(15,4,"" ,0,0,'C');
				...
				$_pdf->Cell($columnas,4,number_format($_x_apagar,2,",",".") ,0,1,'R');$_pdf->Output();}
...
function generar_cheque(){
		...
		$this->_proveedor->cargar_datos_por_id_factura($this->_orden_de_pago->_factura->_nro_factura);
...
$fecha_invertida=$date->format('d/m/Y');return $fecha_invertida;}
function generar_comprobante_de_retencion(){
			...
			$_pdf->Cell(0,4,"R.I.F : J310507066",0,0,'C');$_pdf->Output();}
...
function generar_cheque_completada_ordendepago($id,$tipo){



		if ($tipo=="transferencia") {

			$this->completar_orden($id);
			return;
		}



		$this->_orden_de_pago->cargar_bd($id);



		$this->_proveedor->cargar_datos_por_id_factura($this->_orden_de_pago->_factura->_nro_factura);

		//echo "------------".$this->_orden_de_pago->_id_factura."--------------------";
		$this->_banco->cargar_datos_por_id_cheque($this->_orden_de_pago->_cheque->_id);
...
function completar_orden($orden=false){
	...
	//print_r($this->_orden_de_pago);
		...
		$_pdf->Output();
					...
					public function total_orden(){
						...
						$_pdf->SetFont('Arial','B',10);
						$_pdf->ln(2);
						$ordenes=$this->_orden_de_pago->all();


						$_pdf->Cell(0,6,"lista de ordenes de pago ",0,1,'C');

						$_pdf->ln(20);

							$_pdf->SetFont('Arial','',8);
							$_pdf->Cell(20,4,"nro orde",1,0,'C');
							$_pdf->Cell(50,4,"fecha emicion de la orden",1,0,'C');
							$_pdf->Cell(50,4,"status orden de pago",1,0,'C');
							$_pdf->Cell(20,4,"nro factura",1,0,'C');
							$_pdf->Cell(20,4,"nro control",1,0,'C');
							$_pdf->Cell(20,4,"monto",1,0,'C');
							$_pdf->Cell(20,4,"staus factura",1,0,'C');
							$_pdf->Cell(50,4,"proveedor",1,0,'C');
							$_pdf->Cell(20,4,"rif",1,1,'C');
						...
						for ($i=0; $i < count($ordenes)  ; $i++) { 
							...
							$factura=new factura;
							$factura->cargar_bd($ordenes[$i]["id_factura"]);
							$this->_proveedor->cargar_bd($factura->_id_proveedor);
							//print_r($this->_proveedor);
							//print_r($factura);
							//print_r($ordenes[$i]);
							...
							$_pdf->Cell(20,4,$ordenes[$i]['id_orden_de_pago'],1,0,'C');
							$_pdf->Cell(50,4,$ordenes[$i]['fecha_emicion'],1,0,'C');
							$_pdf->Cell(50,4,$ordenes[$i]['estatus'],1,0,'C');
							$_pdf->Cell(20,4,$factura->_nro_factura,1,0,'C');
							$_pdf->Cell(20,4,$factura->_nro_control,1,0,'C');
							$_pdf->Cell(20,4,$factura->_monto,1,0,'C');
							$_pdf->Cell(20,4,$factura->_estatus,1,0,'C');
							$_pdf->Cell(50,4,$this->_proveedor->_beneficiario,1,0,'C');
							$_pdf->Cell(20,4,$this->_proveedor->_rif,1,1,'C');
						...
						return;
							...
							$_pdf->setx(70);
							$_pdf->Cell(40,6,"lista de ordenes",0,1,'L');
							$_pdf->SetFont('Arial','',6);
							$_pdf->setx(70);
							$_pdf->Cell(30,4,"DOCUMENTO",0,0,'L');
							$_pdf->Cell(2,4,":",0,0,'L');
